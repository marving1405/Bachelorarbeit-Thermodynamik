...
#for v in np.arange(0.8,10,0.1):
#for p2 in np.arange(500000, 2100000, 100000):
#for m_ORC in np.arange(1E-3,40E-3,10E-3):
#for p2 in np.arange(500000, 2000000, 10000):
a = []
for Thoch_H in np.arange(100 + 273.15,210 + 273.15, 1):


    fluid = "REFPROP::PROPANE" #[0.7]&METHANE[0.3]"
    #Thoch_H = 100 + 273.15  # K
    m_ORC = 40E-3  # kg/s
    v = 1
    # beschreibt das Verhältnis von Arbeits- zu Prozessfluid
    m_OEL = v * m_ORC
    ...
    T3 = fsolve(solveT3, 350., args=(Q_zu3, R_ges3, Thoch_H, dTA_3))
    h3 = CP.PropsSI('H', 'T', T3[0], 'P', p2, fluid)
    if h2_sattdampf > h3:
       h3 = CP.PropsSI('H', 'T', T3[0], 'Q', 1, fluid)
...
s_irr = -(Q_zu1/Tlow_H + Q_zu2/Tmittel_H + Q_zu3/Thoch_H - Q_ab1/Te_kuehlmittel1 - Q_ab2/Te_kuehlmittel2)
