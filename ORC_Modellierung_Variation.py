...
#for Thoch_H in np.arange(160 + 273.15,210 + 273.15, 10):
...
Tlow_H = Tlow_L + 20 # heiße Speicherfluidtemperatur immer 20K heißer als nach Abkühlvorgang #TODO warum?
p_Tank1 = 100000  # Pa
m_WASSER = m_OEL


alpha_a_1 = alpha_1P_annulus(p_Tank1, Tlow_L, speicherfluid1, m_WASSER, d_ai, d_aa)
alpha_i_1 = alpha_1P_i(p2, T2, fluid, m_ORC, d_i)

dTA_1 = Tlow_L - T2         #TODO hier Zustand 2!
dTB_1 = Tlow_H - T2_siedend

lambda_Kupfer = 401  # Temperatur am Eingang gewähl #TODO hier muss die Wärmeleitfähigkeit vom Rohr benutzt werden!
A_i = np.pi * d_i
A_a = np.pi * d_ai
R_konv_innen1 = 1 / (A_i * alpha_i_1)
R_konv_aussen1 = 1 / (A_a * alpha_a_1)
R_waermeleitung1 = np.log(d_ai / d_i) / (2 * np.pi * lambda_Kupfer) #TODO hier waren falsche Durchmesser eingesetzt
...
Tmittel_L = T2_siedend + 5  # 50K pinch point #TODO Pinchpoint ist immer die kleinste Temperaturdifferenz, also hier am linken Ende
p_Tank2 = 100000  # Pa
...
cp_oel_Tmittel_L = cp_Oel(Tmittel_L) #TODO ist cp in kJ?
...
R_waermeleitung2 = np.log(d_aa / d_ai) / (2 * np.pi * lambda_Kupfer)
...
R_waermeleitung3 = np.log(d_aa / d_ai) / (2 * np.pi * lambda_Kupfer)
R_ges3 = R_konv_innen3 + R_konv_aussen3 + R_waermeleitung3
l3 = Q_zu3 / ((1/R_ges3) * (dTA_3 - dTB_3) / (np.log(dTA_3 / dTB_3)))
...
R_waermeleitung_k1 = np.log(d_aa / d_ai) / (2 * np.pi * lambda_Kupfer)
...
Te_kuehlmittel2 = T1 - 5 #pinch point temperature = 10K difference
...
plt.title("Nettoleistung über heiße Speichertemperatur", fontsize=14)
plt.xlabel('Thoch_H [K]', fontsize=14)
